...
Instrument* Audio_GetInstrumentInner(s32 fontId, s32 instId) {
    ...
    inst->id = instId;
    sprintf(inst->name, GetInstrumentName(fontId, instId));
#endif
...
s32 Audio_SetFontInstrument(s32 instrumentType, s32 fontId, s32 index, void* value) {
    ...
    switch (instrumentType) {
        case 0:
            ...
            gAudioContext.soundFonts[fontId].drums[index] = (Drum*)value;
            break;
        ...
        default:
            ...
            gAudioContext.soundFonts[fontId].instruments[index] = (Instrument*)value;
            break;
    }
...
Note* Audio_AllocNoteFromDisabled(NotePool* pool, SequenceLayer* layer) {
    auto note = (Note*)AudioSeq_AudioListPopBack(&pool->disabled);
    if (note != NULL) {
...
Note* Audio_AllocNoteFromDecaying(NotePool* pool, SequenceLayer* layer) {
    auto note = (Note*)AudioSeq_AudioListPopBack(&pool->decaying);
    if (note != NULL) {
...
void Audio_NoteInitAll(void) {
    ...
    for (i = 0; i < gAudioContext.numNotes; i++) {
        ...
        note->unk_BC = 0;
        note->synthesisState.synthesisBuffers = (NoteSynthesisBuffers*)AudioHeap_AllocDmaMemory(&gAudioContext.notesAndBuffersPool, 0x1E0);
    }
}
