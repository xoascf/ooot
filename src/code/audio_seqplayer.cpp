...
s32 AudioSeq_SeqChannelSetLayer(SequenceChannel* channel, s32 layerIdx) {
    ...
    if (channel->layers[layerIdx] == NULL) {
        SequenceLayer* layer;
        layer = (SequenceLayer*)AudioSeq_AudioListPopBack(&gAudioContext.layerFreeList);
            ...
            if (command >= 0xF2) {
                ...
                switch (command) {
                    ...
                    case 0xC2:
                        ...
                        channel->dynTable = (u8 (*)[][2])&seqPlayer->seqData[offset];
                        break;
                    case 0xC5:
...
void AudioSeq_InitSequencePlayerChannels(s32 playerIdx) {
    ...
    for (i = 0; i < 0x10; i++) {
        seqPlayer->channels[i] = (SequenceChannel*)AudioHeap_AllocZeroed(&gAudioContext.notesAndBuffersPool, sizeof(SequenceChannel));
