...
//playerDistance to player (unused), baseStrength, time, decay?
void Rumble_Shake2(f32 playerDistance, u8 baseStrength, u8 length, u8 decay) {//Used for bosses and fishing
    ...
    if ((distance_decay < 1000) && (baseStrength != 0) && (decay != 0)) {
        ...
        if (strength > 0) {
            ...
            g_Rumble.unk_10B = length;
            g_Rumble.unk_10C = decay;
        }
    }
}

//distance to player, baseStrength, time, decay??
void Rumble_Shake(f32 playerDistance, u8 baseStrength, u8 length, u8 decay) {
    ...
    if (distance_decay < 1000 && baseStrength != 0 && decay != 0) {
        strengthList = baseStrength - (distance_decay * 255) / 1000;

        for (i = 0; i < 0x40; i++) {
            if (g_Rumble.strengthList[i] == 0) {
                if (strengthList > 0) {
                    g_Rumble.strengthList[i] = strengthList;
