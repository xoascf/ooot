...
void Sram_VerifyAndLoadAllSaves(FileChooseContext* fileChooseCtx, SramContext* sramCtx) {
    ...
    for (slotNum = 0; slotNum < 3; slotNum++) {
        ...
        osSyncPrintf("\nSAVE checksum calculation  j=%x  mmm=%x  ", newChecksum, oldChecksum);
        ...
        if (newChecksum != oldChecksum) {
            ...
            osSyncPrintf("\n(B) SAVE checksum calculation  j=%x  mmm=%x  ", newChecksum, oldChecksum);
...
void Sram_Alloc(GameState* gameState, SramContext* sramCtx) {
    sramCtx->readBuff = (u8*)GameState_Alloc(gameState, SRAM_SIZE, "../z_sram.c", 1294);
