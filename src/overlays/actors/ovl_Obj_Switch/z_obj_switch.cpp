...
void ObjSwitch_InitJntSphCollider(ObjSwitch* pthis, GlobalContext* globalCtx, ColliderJntSphInit* colliderJntSphInit) {
    ...
    func_800D1694(pthis->dyna.actor.world.pos.x,
                  pthis->dyna.actor.world.pos.y + pthis->dyna.actor.shape.yOffset * pthis->dyna.actor.scale.y,
...
Actor* ObjSwitch_SpawnIce(ObjSwitch* pthis, GlobalContext* globalCtx) {
    ...
    return Actor_SpawnAsChild(&globalCtx->actorCtx, thisx, globalCtx, ACTOR_OBJ_ICE_POLY, thisx->world.pos.x,
                              ...
                              thisx->world.rot.z, (pthis->dyna.actor.params >> 8 & 0x3F) << 8);
}
...
void ObjSwitch_FloorPress(ObjSwitch* pthis, GlobalContext* globalCtx) {
    if ((pthis->dyna.actor.params >> 4 & 7) == OBJSWITCH_SUBTYPE_FLOOR_3 || !pthis->cooldownOn ||
        func_8005B198() == pthis->dyna.actor.category || pthis->cooldownTimer <= 0) {
...
void ObjSwitch_FloorRelease(ObjSwitch* pthis, GlobalContext* globalCtx) {
    ...
    if (((subType != OBJSWITCH_SUBTYPE_FLOOR_1) && (subType != OBJSWITCH_SUBTYPE_FLOOR_3)) || !pthis->cooldownOn ||
        func_8005B198() == pthis->dyna.actor.category || pthis->cooldownTimer <= 0) {
        pthis->dyna.actor.scale.y += 99.0f / 2000.0f;
...
void ObjSwitch_EyeOpening(ObjSwitch* pthis, GlobalContext* globalCtx) {
    if ((pthis->dyna.actor.params >> 4 & 7) != OBJSWITCH_SUBTYPE_EYE_1 || !pthis->cooldownOn ||
        func_8005B198() == pthis->dyna.actor.category || pthis->cooldownTimer <= 0) {
        pthis->eyeTexIndex--;
...
void ObjSwitch_CrystalTurnOff(ObjSwitch* pthis, GlobalContext* globalCtx) {
    if ((pthis->dyna.actor.params >> 4 & 7) != OBJSWITCH_SUBTYPE_CRYSTAL_1 || !pthis->cooldownOn ||
        func_8005B198() == pthis->dyna.actor.category || pthis->cooldownTimer <= 0) {
        ObjSwitch_CrystalOffInit(pthis);
        ObjSwitch_UpdateTwoTexScrollXY(pthis);
        Audio_PlayActorSound2(&pthis->dyna.actor, NA_SE_EV_DIAMOND_SWITCH);
