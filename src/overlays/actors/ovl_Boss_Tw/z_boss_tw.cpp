...
void BossTw_Init(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if (pthis->actor.params == TW_KOTAKE) {
        ...
        if (Flags_GetClear(globalCtx, globalCtx->roomCtx.curRoom.num)) {
            ...
            Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_DOOR_WARP1, 600.0f, 230.0f, 0.0f, 0,
                               0, 0, WARP_DUNGEON_ADULT);
            Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_ITEM_B_HEART, -600.0f, 230.0f, 0.0f, 0, 0, 0, 0);
        } else {
            sKotakePtr = (BossTw*)Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_BOSS_TW,
                                                     pthis->actor.world.pos.x, pthis->actor.world.pos.y,
                                                     pthis->actor.world.pos.z, 0, 0, 0, TW_KOTAKE);
            sKoumePtr = (BossTw*)Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_BOSS_TW,
                                                    pthis->actor.world.pos.x, pthis->actor.world.pos.y,
                                                    pthis->actor.world.pos.z, 0, 0, 0, TW_KOUME);
...
void BossTw_SpawnGroundBlast(BossTw* pthis, GlobalContext* globalCtx, s16 blastType) {
    ...
    if (blastType == 1) {
        sGroundBlastType = 1;
        groundBlast = (BossTw*)Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_BOSS_TW,
                                                  pthis->groundBlastPos.x, pthis->groundBlastPos.y,
                                                  pthis->groundBlastPos.z, 0, 0, 0, TW_FIRE_BLAST_GROUND);
        if (groundBlast != NULL) {
        ...
        groundBlast = (BossTw*)Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_BOSS_TW,
                                                  pthis->groundBlastPos.x, pthis->groundBlastPos.y,
                                                  pthis->groundBlastPos.z, 0, 0, 0, TW_ICE_BLAST_GROUND);
        if (groundBlast != NULL) {
            if (sTwinrovaPtr->actionFunc == BossTw_Wait) {
                groundBlast->timers[0] = 100;
            } else {
...
s32 BossTw_BeamHitPlayerCheck(BossTw* pthis, GlobalContext* globalCtx) {
    ...
    if (fabsf(beamDistFromPlayer.x) < 20.0f && fabsf(beamDistFromPlayer.y) < 50.0f && beamDistFromPlayer.z > 100.0f &&
        beamDistFromPlayer.z <= pthis->beamDist) {
...
s32 BossTw_BeamReflHitCheck(BossTw* pthis, Vec3f* pos) {
    ...
    if (fabsf(beamDistFromTarget.x) < 50.0f && fabsf(beamDistFromTarget.y) < 50.0f && beamDistFromTarget.z > 100.0f &&
        beamDistFromTarget.z <= pthis->beamReflectionDist) {
            ...
            case 0:
                if (pthis->timers[0] != 0) {
                    ...
                    if (beamReflection == 1) {
                        ...
                        if (pthis->csState1 == 0) {
                            Math_ApproachF(&pthis->beamDist, 2.0f * sqrtf(SQ(xDiff) + SQ(yDiff) + SQ(zDiff)), 1.0f,
                                           40.0f);
                        }
                    }
                }

                SkinMatrix_Vec3fMtxFMultXYZW(&globalCtx->viewProjectionMtxF, &pthis->beamReflectionOrigin,
                                             &pthis->unk_54C, &pthis->actor.projectedW);
            ...
            case 1:
                if (CHECK_BTN_ALL(input->cur.button, BTN_R)) {
                    ...
                    if ((pthis->work[CS_TIMER_1] % 4) == 0) {
                        BossTw_AddRingEffect(globalCtx, &player->bodyPartsPos[15], 0.5f, 3.0f, 0xFF, pthis->actor.params,
                                             1, 150);
                    }
                } else {
                    pthis->beamShootState = 0;
                ...
                SkinMatrix_Vec3fMtxFMultXYZW(&globalCtx->viewProjectionMtxF, &pthis->unk_530, &pthis->unk_558,
    ...
    if (pthis->beamShootState == 1) {
        ...
        if (pthis->csState1 == 0) {
            ...
            for (i = 0; i < 200; i++) {
                ...
                if (floorY >= 0.0f) {
                    if ((pthis->groundBlastPos.y != 35.0f) && (0.0f < pthis->beamReflectionPitch) &&
                        (pthis->timers[0] != 0)) {
                        pthis->csState1 = 1;
                        ...
                        for (i = 0; i < 5; i++) {
                            ...
                            BossTw_AddFlameEffect(globalCtx, &pthis->unk_530, &velocity, &accel,
                                                  Rand_ZeroFloat(10.0f) + 25.0f, pthis->actor.params);
                        }
...
void BossTw_HitByBeam(BossTw* pthis, GlobalContext* globalCtx) {
    ...
    if ((pthis->work[CS_TIMER_1] % 4) == 0) {
        ...
        BossTw_AddDmgCloud(globalCtx, pthis->actor.params + 2, &pos, &velocity, &accel, Rand_ZeroFloat(10.0f) + 15.0f, 0,
                           0, 150);
    }
    ...
    Math_ApproachF(&pthis->actor.world.pos.y, ((Math_SinS(pthis->work[CS_TIMER_1] * 1500) * 20.0f) + 350.0f) + 50.0f,
                   0.1f, pthis->actor.speedXZ);
...
void BossTw_Wait(BossTw* pthis, GlobalContext* globalCtx) {
    if ((pthis->actor.params == TW_TWINROVA) && (sKoumePtr->actionFunc == BossTw_FlyTo) &&
...
void BossTw_TwinrovaIntroCS(BossTw* pthis, GlobalContext* globalCtx) {
                ...
                if (pthis->work[CS_TIMER_1] >= 35) {
                    if (pthis->work[CS_TIMER_1] < 50) {
                        Math_ApproachF(&sKoumePtr->actor.scale.x,
                                       ((Math_SinS(pthis->work[CS_TIMER_1] * 0x4200) * 20.0f) / 10000.0f) + 0.024999999f,
                                       1.0f, 0.005f);
                    } else {
                        if (pthis->work[CS_TIMER_1] == 50) {
        ...
        case 20:
            ...
            if (pthis->work[CS_TIMER_1] < 200) {
                ...
                sKotakePtr->actor.shape.rot.y = sKotakePtr->actor.world.rot.y =
                    ((pthis->workf[UNK_F9] / M_PI) * 32768.0f) + 32768.0f;
    ...
    if (pthis->subCamId != 0) {
        if (updateCam) {
            Math_ApproachF(&pthis->subCamEye.x, pthis->subCamEyeTarget.x, pthis->subCamDistStep,
                           pthis->subCamEyeStep.x * pthis->subCamUpdateRate);
            Math_ApproachF(&pthis->subCamEye.y, pthis->subCamEyeTarget.y, pthis->subCamDistStep,
                           pthis->subCamEyeStep.y * pthis->subCamUpdateRate);
            Math_ApproachF(&pthis->subCamEye.z, pthis->subCamEyeTarget.z, pthis->subCamDistStep,
                           pthis->subCamEyeStep.z * pthis->subCamUpdateRate);
            Math_ApproachF(&pthis->subCamAt.x, pthis->subCamAtTarget.x, pthis->subCamDistStep,
                           pthis->subCamAtStep.x * pthis->subCamUpdateRate);
            Math_ApproachF(&pthis->subCamAt.y, pthis->subCamAtTarget.y, pthis->subCamDistStep,
                           pthis->subCamAtStep.y * pthis->subCamUpdateRate);
            Math_ApproachF(&pthis->subCamAt.z, pthis->subCamAtTarget.z, pthis->subCamDistStep,
                           pthis->subCamAtStep.z * pthis->subCamUpdateRate);
        }
...
void BossTw_DeathBall(BossTw* pthis, GlobalContext* globalCtx) {
    ...
    Math_ApproachS(&pthis->actor.world.rot.x, Math_FAtan2F(yDiff, sqrtf(SQ(xDiff) + SQ(zDiff))) * (32768 / M_PI), 5,
                   pthis->rotateSpeed);
...
void BossTw_DeathCSMsgSfx(BossTw* pthis, GlobalContext* globalCtx) {
    ...
    if (pthis->work[CS_TIMER_2] >= 150) {
        Math_ApproachF(&sKoumePtr->workf[UNK_F17], (Math_SinS(pthis->work[CS_TIMER_1] * 2000) * 0.05f) + 0.4f, 0.1f,
                       0.01f);
        Math_ApproachF(&sKotakePtr->workf[UNK_F17], (Math_CosS(pthis->work[CS_TIMER_1] * 1700) * 0.05f) + 0.4f, 0.1f,
                       0.01f);
        ...
        case 0:
            ...
            if (pthis->work[CS_TIMER_1] >= 55) {
                ...
                if (pthis->work[CS_TIMER_1] == 135) {
                    ...
                    Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_BOSS_TW,
    ...
    switch (pthis->csState2) {
        ...
        case 3:
            ...
            if (pthis->work[CS_TIMER_2] < 150) {
                ...
                Math_ApproachF(&globalCtx->envCtx.unk_D8, (Math_SinS(pthis->work[CS_TIMER_2] * 4096) / 4.0f) + 0.75f,
                               1.0f, 0.1f);
            }
            ...
            if (pthis->work[CS_TIMER_2] >= 1020) {
                ...
                Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_DOOR_WARP1, 600.0f, 230.0f,
                ...
                if (&pthis->subCamEye) {} // fixes regalloc, may be fake
...
void BossTw_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (pthis->actionFunc == BossTw_FlyTo || pthis->actionFunc == BossTw_Spin ||
        pthis->actionFunc == BossTw_TurnToPlayer) {
        if ((s16)(player->actor.shape.rot.y - pthis->actor.yawTowardsPlayer + 0x8000) < 0x1000 &&
            (s16)(player->actor.shape.rot.y - pthis->actor.yawTowardsPlayer + 0x8000) > -0x1000 && player->unk_A73) {
...
void BossTw_TwinrovaUpdate(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if (pthis->actionFunc != BossTw_TwinrovaShootBlast && pthis->actionFunc != BossTw_TwinrovaChargeBlast &&
        pthis->visible && pthis->unk_5F8 == 0 &&
        (s16)(player->actor.shape.rot.y - pthis->actor.yawTowardsPlayer + 0x8000) < 0x1000 &&
    ...
    switch (sEnvType) {
        ...
        case 2:
            globalCtx->envCtx.unk_BD = 2;
            Math_ApproachF(&globalCtx->envCtx.unk_D8, (Math_SinS(pthis->work[CS_TIMER_1] * 0x3000) * 0.03f) + 0.5f, 1.0f,
                           0.05f);
            break;
        case 3:
            globalCtx->envCtx.unk_BD = 3;
        ...
        case 4:
            globalCtx->envCtx.unk_BD = 2;
            Math_ApproachF(&globalCtx->envCtx.unk_D8, (Math_SinS(pthis->work[CS_TIMER_1] * 0x3E00) * 0.05f) + 0.95f,
                           1.0f, 0.1f);
            break;
        case 5:
            globalCtx->envCtx.unk_BD = 0;
...
void func_80941BC0(BossTw* pthis, GlobalContext* globalCtx) {
    ...
    gSPSegment(POLY_XLU_DISP++, 8,
               Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, 0, 0, 0x20, 0x40, 1, (u32)pthis->workf[UNK_F16] & 0x3F,
                                (pthis->work[CS_TIMER_2] * 4) & 0x3F, 0x10, 0x10));
    Matrix_Push();
    ...
    gSPSegment(POLY_XLU_DISP++, 0xD,
               Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, pthis->work[CS_TIMER_2] & 0x7F,
                                (pthis->work[CS_TIMER_2] * 8) & 0xFF, 0x20, 0x40, 1,
                                (-pthis->work[CS_TIMER_2] * 2) & 0x3F, 0, 0x10, 0x10));
...
void func_80942180(BossTw* pthis, GlobalContext* globalCtx) {
    ...
    gSPSegment(POLY_XLU_DISP++, 8,
               Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, (-pthis->work[CS_TIMER_1]) & 0x7F, 0, 0x20, 0x20, 1,
                                (pthis->work[CS_TIMER_1] * 2) & 0x7F, 0, 0x20, 0x20));
    ...
    gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_boss_tw.c", 6514),
              G_MTX_LOAD | G_MTX_MODELVIEW | G_MTX_NOPUSH);
    gSPSegment(POLY_XLU_DISP++, 8,
               Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, pthis->work[CS_TIMER_1] & 0x7F,
                                (-pthis->work[CS_TIMER_1] * 6) & 0xFF, 0x20, 0x40, 1,
                                (pthis->work[CS_TIMER_1] * 2) & 0x7F, (-pthis->work[CS_TIMER_1] * 6) & 0xFF, 0x20, 0x40));
    ...
    gSPSegment(POLY_XLU_DISP++, 8,
               Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, (-pthis->work[CS_TIMER_1] * 3) & 0x7F, 0, 0x20, 0x20, 1, 0,
                                (-pthis->work[CS_TIMER_1] * 10) & 0xFF, 0x20, 0x40));
...
void func_809426F0(BossTw* pthis, GlobalContext* globalCtx) {
    ...
    gSPSegment(POLY_XLU_DISP++, 8,
               Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, 0, (u8)(-pthis->work[CS_TIMER_2] * 15), 0x20, 0x40, 1, 0, 0,
                                0x40, 0x40));
    Matrix_Push();
    Matrix_Translate(0.0f, 0.0f, 5000.0f, MTXMODE_APPLY);
    Matrix_Scale(pthis->spawnPortalScale / 2000.0f, pthis->spawnPortalScale / 2000.0f, pthis->spawnPortalScale / 2000.0f,
                 MTXMODE_APPLY);
    ...
    for (i = 0; i < 8; i++) {
        ...
        gSPSegment(POLY_XLU_DISP++, 8,
                   Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, ((pthis->work[CS_TIMER_2] * 3) + (i * 10)) & 0x7F,
                                    (u8)((-pthis->work[CS_TIMER_2] * 15) + (i * 50)), 0x20, 0x40, 1, 0, 0, 0x20, 0x20));
...
void func_80942C70(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (pthis->beamDist != 0.0f) {
        ...
        gSPSegment(POLY_XLU_DISP++, 0xC,
                   Gfx_TexScroll(globalCtx->state.gfxCtx, 0, (u8)(pthis->work[CS_TIMER_1] * -0xF), 0x20, 0x40));
        ...
        if (pthis->beamReflectionDist > 10.0f) {
            ...
            Matrix_RotateX(pthis->beamReflectionPitch, MTXMODE_APPLY);
            Matrix_RotateZ(pthis->beamRoll, MTXMODE_APPLY);
            Matrix_Scale(pthis->beamScale, pthis->beamScale, (pthis->beamReflectionDist * 0.01f * 100.0f) / 20000.0f,
                         MTXMODE_APPLY);
...
void func_80943028(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Matrix_Scale((pthis->actor.scale.x * 4000.0f) / 100.0f, 1.0f, (pthis->actor.scale.x * 4000.0f) / 100.0f,
                 MTXMODE_APPLY);
    ...
    if (pthis->visible) {
        ...
        gSPSegment(POLY_XLU_DISP++, 8,
                   Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, (s16)pthis->workf[OUTR_CRWN_TX_X1] & 0x7F,
                                    ...
                                    (s16)pthis->workf[OUTR_CRWN_TX_X2] & 0x7F, (s16)pthis->workf[OUTR_CRWN_TX_Y2] & 0xFF,
                                    0x20, 0x40));

        if (pthis->actor.params == TW_KOTAKE) {
            gSPSegment(POLY_XLU_DISP++, 9,
                       Gfx_TexScroll(globalCtx->state.gfxCtx, (s16)pthis->workf[INNR_CRWN_TX_X1] & 0x7F,
                                     (s16)pthis->workf[INNR_CRWN_TX_Y1] & 0xFF, 0x20, 0x40));
        } else {
            gSPSegment(POLY_XLU_DISP++, 9,
                       Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, (s16)pthis->workf[INNR_CRWN_TX_X1] & 0x7F,
                                        (s16)pthis->workf[INNR_CRWN_TX_Y1] & 0x7F, 0x20, 0x20, 1,
                                        (s16)pthis->workf[INNR_CRWN_TX_X2] & 0x7F,
                                        (s16)pthis->workf[INNR_CRWN_TX_Y2] & 0xFF, 0x20, 0x40));
        }
        ...
        if (pthis->work[FOG_TIMER] & 2) {
            ...
            POLY_OPA_DISP = Gfx_SetFog(POLY_OPA_DISP, (u32)pthis->fogR, (u32)pthis->fogG, (u32)pthis->fogB, 0,
                                       pthis->fogNear, pthis->fogFar);
        }

        Matrix_Push();
        SkelAnime_DrawFlexOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable,
                              pthis->skelAnime.dListCount, BossTw_OverrideLimbDraw, BossTw_PostLimbDraw, pthis);
        Matrix_Pop();
        ...
        if (pthis->workf[UNK_F9] > 0.0f) {
            if (pthis->workf[UNK_F11] > 0.0f) {
                ...
                if ((fabsf(diff.y) < 10.0f) && (player->actor.bgCheckFlags & 1) &&
                    (sqrtf(SQ(diff.x) + SQ(diff.z)) < (pthis->workf[UNK_F12] * 4600.0f)) && (sFreezeState == 0) &&
                    (pthis->workf[UNK_F11] > 200.0f)) {
                    sFreezeState = 1;
    ...
    switch (limbIndex) {
        case 21:
            gSPSegment(POLY_OPA_DISP++, 0xC,
                       Gfx_TexScroll(globalCtx->state.gfxCtx, 0, (s16)(f32)pthis->work[CS_TIMER_1], 8, 8));
            ...
            *dList = NULL;
            gSPSegment(POLY_XLU_DISP++, 0xA,
                       Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, 0, 0, 0x20, 0x20, 1, 0,
                                        -pthis->work[CS_TIMER_1] * 0xF, 0x20, 0x40));
            break;
        case 18:
        case 42:
            *dList = NULL;
            gSPSegment(POLY_XLU_DISP++, 0xB,
                       Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, 0, 0, 0x20, 0x20, 1, 0,
                                        -pthis->work[CS_TIMER_1] * 0xA, 0x20, 0x40));
            break;
        case 16:
        case 32:
            *dList = NULL;
            gSPSegment(POLY_XLU_DISP++, 8,
                       Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, 0, 0, 0x20, 0x20, 1, pthis->work[CS_TIMER_1],
                                        -pthis->work[CS_TIMER_1] * 7, 0x20, 0x40));
            break;
        case 15:
        case 31:
            *dList = NULL;
            gSPSegment(POLY_XLU_DISP++, 9,
                       Gfx_TexScroll(globalCtx->state.gfxCtx, 0, pthis->work[CS_TIMER_1], 0x20, 0x40));
            break;
        case 19:
            if (pthis->unk_5F8 != 0) {
...
void BossTw_SpawnPortalDraw(BossTw* pthis, GlobalContext* globalCtx) {
    ...
    gSPSegment(
        POLY_XLU_DISP++, 8,
        Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, 0, -pthis->work[CS_TIMER_1] * 15, 0x20, 0x40, 1, 0, 0, 0x40, 0x40));
...
void BossTw_TwinrovaDraw(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if (pthis->visible) {
        ...
        POLY_OPA_DISP = (pthis->work[FOG_TIMER] & 2) ? Gfx_SetFog2(POLY_OPA_DISP, 255, 50, 0, 0, 900, 1099)
                                                    : Gfx_SetFog2(POLY_OPA_DISP, (u32)pthis->fogR, (u32)pthis->fogG,
                                                                  (u32)pthis->fogB, 0, pthis->fogNear, pthis->fogFar);

        Matrix_Push();
        SkelAnime_DrawFlexOpa(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable,
                              pthis->skelAnime.dListCount, BossTw_TwinrovaOverrideLimbDraw, BossTw_TwinrovaPostLimbDraw,
                              thisx);
        Matrix_Pop();
                    ...
                    if (pthis->groundBlastPos.y >= 0.0f) {
                        if (pthis->groundBlastPos.y != 35.0f) {
                            ...
                            for (i = 0; i < 50; i++) {
                                ...
                                accel.y = 0.0f;
                                accel.z = 0.0f;
                                BossTw_AddFlameEffect(globalCtx, &pthis->actor.world.pos, &velocity, &accel,
                                                      Rand_ZeroFloat(10.0f) + 25.0f, pthis->blastType);
                            }
    ...
    switch (pthis->actor.params) {
        case TW_FIRE_BLAST:
            ...
            for (i = 9; i >= 0; i--) {
                gSPSegment(POLY_XLU_DISP++, 8,
                           Gfx_TwoTexScroll(
                               globalCtx->state.gfxCtx, 0, ((pthis->work[CS_TIMER_1] * 3) + (i * 10)) & 0x7F,
                               ((-pthis->work[CS_TIMER_1] * 15) + (i * 50)) & 0xFF, 0x20, 0x40, 1, 0, 0, 0x20, 0x20));
                tailIdx = ((pthis->work[TAIL_IDX] - i) + 30) % 30;
                Matrix_Translate(pthis->blastTailPos[tailIdx].x, pthis->blastTailPos[tailIdx].y,
                                 pthis->blastTailPos[tailIdx].z, MTXMODE_NEW);
        ...
        case TW_ICE_BLAST:
            ...
            for (i = 9; i >= 0; i--) {
                gSPSegment(POLY_XLU_DISP++, 8,
                           Gfx_TwoTexScroll(
                               globalCtx->state.gfxCtx, 0, ((pthis->work[CS_TIMER_1] * 3) + (i * 0xA)) & 0x7F,
                               (u8)((-pthis->work[CS_TIMER_1] * 0xF) + (i * 50)), 0x20, 0x40, 1, 0, 0, 0x20, 0x20));
                tailIdx = ((pthis->work[TAIL_IDX] - i) + 30) % 30;
                Matrix_Translate(pthis->blastTailPos[tailIdx].x, pthis->blastTailPos[tailIdx].y,
                                 pthis->blastTailPos[tailIdx].z, MTXMODE_NEW);
                scaleFactor = 1.0f - (i * 0.09f);
...
void BossTw_DrawDeathBall(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if (pthis->actor.params == TW_DEATHBALL_KOUME) {
        ...
        for (i = 9; i >= 0; i--) {
            gSPSegment(POLY_XLU_DISP++, 8,
                       Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, (((pthis->work[CS_TIMER_1] * 3) + (i * 0xA))) & 0x7F,
                                        (u8)((-pthis->work[CS_TIMER_1] * 0xF) + (i * 50)), 0x20, 0x40, 1, 0, 0, 0x20,
                                        0x20));
            tailIdx = ((pthis->work[TAIL_IDX] - i) + 30) % 30;
            Matrix_Translate(pthis->blastTailPos[tailIdx].x, pthis->blastTailPos[tailIdx].y,
                             pthis->blastTailPos[tailIdx].z, MTXMODE_NEW);
...
void BossTw_TwinrovaShootBlast(BossTw* pthis, GlobalContext* globalCtx) {
    ...
    if (Animation_OnFrame(&pthis->skelAnime, 12.0f)) {
        ...
        twMagic =
            (BossTw*)Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_BOSS_TW, magicSpawnPos->x,
...
void BossTw_TwinrovaFly(BossTw* pthis, GlobalContext* globalCtx) {
    ...
    xzDist = sqrtf(SQ(xDiff) + SQ(zDiff));
    Math_ApproachS(&pthis->actor.world.rot.x,
