...
void ElfMsg_CallNaviCuboid(ElfMsg* pthis, GlobalContext* globalCtx) {
    ...
    if ((fabsf(player->actor.world.pos.x - pthis->actor.world.pos.x) < (100.0f * pthis->actor.scale.x)) &&
        ...
        (fabsf(player->actor.world.pos.z - pthis->actor.world.pos.z) < (100.0f * pthis->actor.scale.z))) {
...
void ElfMsg_CallNaviCylinder(ElfMsg* pthis, GlobalContext* globalCtx) {
    ...
    if (ElfMsg_WithinXZDistance(&player->actor.world.pos, &pthis->actor.world.pos, pthis->actor.scale.x * 100.0f) &&
        (pthis->actor.world.pos.y <= player->actor.world.pos.y) &&
        ((player->actor.world.pos.y - pthis->actor.world.pos.y) < (100.0f * pthis->actor.scale.y))) {
        player->naviTextId = ElfMsg_GetMessageId(pthis);
        navi->elfMsg = pthis;
