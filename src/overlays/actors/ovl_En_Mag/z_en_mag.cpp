...
void EnMag_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (pthis->globalState == MAG_STATE_FADE_IN) {
        if (pthis->effectFadeInState == 0) {
            ...
            pthis->effectPrimColor[0] += (255.0f / 40.0f);
            pthis->effectPrimColor[1] += ((255.0f - 100.0f) / 40.0f);
            pthis->effectPrimColor[2] += ((255.0f - 170.0f) / 40.0f);
            pthis->effectEnvColor[0] += (255.0f / 40.0f);
            pthis->effectEnvColor[1] += ((255.0f - 100.0f) / 40.0f);
            ...
            pthis->effectPrimColor[2] += ((170.0f - 255.0f) / 40.0f);
            pthis->effectEnvColor[1] += ((100.0f - 255.0f) / 40.0f);
...
void EnMag_DrawInner(Actor* thisx, GlobalContext* globalCtx, Gfx** gfxp) {
    ...
    if ((s16)pthis->copyrightAlpha != 0) {
        gDPLoadTextureBlock(gfx++, gTitleCopyrightTex, G_IM_FMT_IA, G_IM_SIZ_8b, 128, 16, 0, G_TX_NOMIRROR | G_TX_CLAMP,
                            G_TX_NOMIRROR | G_TX_CLAMP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
        gSPTextureRectangle(gfx++, 94 << 2, 198 << 2, (94 + 128) << 2, (198 + 16) << 2, G_TX_RENDERTILE, 0, 0, 1 << 10,
                            1 << 10);
    }
