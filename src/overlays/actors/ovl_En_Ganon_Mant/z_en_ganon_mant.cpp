...
#include "overlays/ovl_En_Ganon_Mant/ovl_En_Ganon_Mant.cpp"
 ...
 */
void EnGanonMant_Tear(EnGanonMant* pthis) {
    s32 pad;
    s16 i;
    s16 areaX;
    s16 areaY;
    ...
    for (i = 0; i < 4; i++) {
        pthis->strands[(s16)Rand_ZeroFloat(GANON_MANT_NUM_STRANDS - 0.1f)]
...
void EnGanonMant_UpdateStrand(GlobalContext* globalCtx, EnGanonMant* pthis, Vec3f* root, Vec3f* pos, Vec3f* nextPos,
    ...
    for (i = 0; i < GANON_MANT_NUM_JOINTS; i++, pos++, vel++, rot++, nextPos++) {
        if (i == 0) {
            ...
            Matrix_RotateY(pthis->baseYaw, MTXMODE_NEW);
            Matrix_MultVec3f(&delta, &backSwayOffset);

            // Push the cloak out to either side, in a swaying manner
            delta.x = cosf((strandNum * M_PI) / (GANON_MANT_NUM_STRANDS - 1.0f)) * pthis->sideSwayMagnitude *
            ...
            xDiff = pos->x - pthis->actor.world.pos.x;
            zDiff = pos->z - pthis->actor.world.pos.z;
            if (sqrtf(SQ(xDiff) + SQ(zDiff)) < (sDistMultipliers[i] * pthis->minDist)) {
                yaw = Math_Atan2F(zDiff, xDiff);
                delta.z = pthis->minDist * sDistMultipliers[i];
                Matrix_RotateY(yaw, MTXMODE_NEW);
                Matrix_MultVec3f(&delta, &posStep);
                pos->x = pthis->actor.world.pos.x + posStep.x;
                pos->z = pthis->actor.world.pos.z + posStep.z;
            }
 ...
 */
void EnGanonMant_UpdateVertices(EnGanonMant* pthis) {
    s16 i;
    Vtx* vtx;
    Vtx* vertices;
    MantStrand* strand;
...
void EnGanonMant_Update(Actor* thisx, GlobalContext* globalCtx) {
    EnGanonMant* pthis = (EnGanonMant*)thisx;
    BossGanon* ganon = (BossGanon*)pthis->actor.parent;

    pthis->updateHasRun = true;
    pthis->frameTimer++;

    if (pthis->attachLeftArmTimer == 0.0f) {
    } else {
        pthis->attachLeftArmTimer -= 1.0f;
    }
    if (pthis->attachRightArmTimer != 0.0f) {
        pthis->attachRightArmTimer -= 1.0f;
    }
    if (pthis->attachShouldersTimer != 0.0f) {
        pthis->attachShouldersTimer -= 1.0f;
    }

    pthis->actor.shape.rot.y = ganon->actor.shape.rot.y;

    if (pthis->tearTimer != 0) {
        pthis->tearTimer--;
        EnGanonMant_Tear(pthis);
    }
}

void EnGanonMant_DrawCloak(GlobalContext* globalCtx, EnGanonMant* pthis) {
    s32 pad;
...
void EnGanonMant_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (pthis->updateHasRun) {
        // Only run pthis if update has run since last draw

        // Choose endpoints
        if (pthis->attachRightArmTimer != 0.0f) {
            rightPos = &pthis->rightForearmPos;
            leftPos = &pthis->leftShoulderPos;
            pthis->gravity = -13.0f;
        } else if (pthis->attachLeftArmTimer != 0.0f) {
            rightPos = &pthis->rightShoulderPos;
            leftPos = &pthis->leftForearmPos;
            pthis->gravity = -13.0f;
        } else if (pthis->attachShouldersTimer != 0.0f) {
            rightPos = &pthis->rightShoulderPos;
            leftPos = &pthis->leftShoulderPos;
            pthis->gravity = -3.0f;
        } else {
        ...
        for (strandIdx = 0; strandIdx < GANON_MANT_NUM_STRANDS; strandIdx++) {
            ...
            pthis->strands[strandIdx].root.x = midpoint.x + strandDivPos.x;
            pthis->strands[strandIdx].root.y = midpoint.y + strandDivPos.y;
            pthis->strands[strandIdx].root.z = midpoint.z + strandDivPos.z;
            ...
            EnGanonMant_UpdateStrand(globalCtx, pthis, &pthis->strands[strandIdx].root, pthis->strands[strandIdx].joints,
                                     pthis->strands[nextStrandIdx].joints, pthis->strands[strandIdx].rotations,
                                     pthis->strands[strandIdx].velocities, strandIdx);
            Matrix_Pop();
        }
        EnGanonMant_UpdateVertices(pthis);
        pthis->updateHasRun = false;
    }
