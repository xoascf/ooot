...
void func_80AE2C1C(EnRd* pthis, GlobalContext* globalCtx) {
    ...
    if (!pthis->unk_307 && (Actor_WorldDistXYZToActor(&pthis->actor, &player->actor) <= 45.0f) &&
        Actor_IsFacingPlayer(&pthis->actor, 0x38E3)) {
...
void func_80AE4114(EnRd* pthis, GlobalContext* globalCtx) {
    ...
    if ((gSaveContext.sunsSongState != SUNSSONG_INACTIVE) && (pthis->actor.shape.rot.x == 0) && (pthis->unk_318 == 0) &&
        (pthis->unk_31B != 9) && (pthis->unk_31B != 10) && (pthis->unk_31B != 1)) {
        func_80AE3DE4(pthis);
        return;
    }
...
void EnRd_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (pthis->unk_314 == 0xFF) {
        ...
        POLY_OPA_DISP = SkelAnime_DrawFlex(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable,
                                           pthis->skelAnime.dListCount, EnRd_OverrideLimbDraw, EnRd_PostLimbDraw, pthis,
                                           POLY_OPA_DISP);
        ...
        POLY_XLU_DISP =
            SkelAnime_DrawFlex(globalCtx, pthis->skelAnime.skeleton, pthis->skelAnime.jointTable,
                               pthis->skelAnime.dListCount, EnRd_OverrideLimbDraw, NULL, pthis, POLY_XLU_DISP);
